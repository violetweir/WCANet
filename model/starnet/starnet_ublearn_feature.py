...
class Block(nn.Module):
    def __init__(self, dim, ffn_ratio, drop_path=0., wt_type='db1', learnable_wavelet=False,stage=0):
        ...
        # 注意：这里WTAttn现在支持return_features
...
class FSANet(nn.Module):
    ...
    def forward_with_wtattn_features(self, x, stage_idx=0):
        ...
        # 注意：调用的是 WTAttn 实例本身，并传入 return_features=True
        ...
        for i in range(stage_idx + 1, 4): # 修正循环范围，从 stage_idx+1 开始
             x_rest = blocks_list[i](x_after_wtattn)
             x_after_wtattn = x_rest # 更新 x_rest 以便传递给下一个 stage
...
def FSANet_64_T1_UNLEARN(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_StarAttn_T1_64):
    model = FSANet(num_classes=num_classes, distillation=distillation, **model_cfg)
    return model

#@MODEL.register_module
#运算量：103.652M, 参数量：2.634M
def FSANet_64_T2_UNLEARN(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_StarAttn_T2_64):
    model = FSANet(num_classes=num_classes, distillation=distillation, **model_cfg)
    return model

##@MODEL.register_module
#运算量：103.652M, 参数量：2.634M
def FSANet_64_T2d5_UNLEARN(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_StarAttn_T2d5_64):
    model = FSANet(num_classes=num_classes, distillation=distillation, **model_cfg)
    return model

#@MODEL.register_module
#运算量：209.937M, 参数量：5.235M
def FSANet_64_T3_UNLEARN(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_StarAttn_T3_64):
    model = FSANet(num_classes=num_classes, distillation=distillation, **model_cfg)
    return model

@MODEL.register_module
#运算量：297.404M, 参数量：7.698M
def FSANet_64_T4_UNLEARN_lhhh(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_StarAttn_T4_64):
    model = FSANet(num_classes=num_classes, distillation=distillation, **model_cfg)
    return model

#@MODEL.register_module
#运算量：485.102M, 参数量：11.905M
def FSANet_64_T5_UNLEARN(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_StarAttn_T5_64):
    model = FSANet(num_classes=num_classes, distillation=distillation, **model_cfg)
    return model

#@MODEL.register_module
#运算量：297.404M, 参数量：7.698M
def FSANet_64_T6_UNLEARN(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_StarAttn_T6_64):
    model = FSANet(num_classes=num_classes, distillation=distillation, **model_cfg)
    return model
...
#@MODEL.register_module
#运算量：297.404M, 参数量：7.698M
def FSANet_64_T8_UNLEARN(num_classes=1000, pretrained=False, distillation=False, fuse=False, pretrained_cfg=None, model_cfg=CFG_StarAttn_T8_64):
    model = FSANet(num_classes=num_classes, distillation=distillation, **model_cfg)
    return model
...
if __name__ == "__main__":
    ...
    # --- 示例用法 ---
    # 获取第0个stage的WTAttn特征
    # final_out, features = model.forward_with_wtattn_features(x, stage_idx=0)
    # print(f"Final Output Shape: {final_out.shape}")
    # print(f"Feature Keys: {list(features.keys())}")
    # print(f"Input Shape: {features['input'].shape}")
    # print(f"LL Shape: {features['ll'].shape}")
    # print(f"Attn Shape: {features['attn'].shape}")
