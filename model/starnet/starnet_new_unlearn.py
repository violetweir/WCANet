...
class WTAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply convolutions
        hh_conv = self.lh_conv(hh)
        lh_conv = self.hl_conv(lh)
        ...
        # Combine results
        attn = (self.act((hh_conv * lh_conv )) * ll_conv )+ ll
        wt_map = torch.cat([attn.unsqueeze(2), lh_conv.unsqueeze(2), hl.unsqueeze(2), hh_conv.unsqueeze(2)], dim=2)
